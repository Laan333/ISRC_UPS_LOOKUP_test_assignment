...
- **Orchestration:** `asyncio.gather` across providers; the overall response stays **200** when the HTTP handler succeeds. Providers that end with **`found: false` and a non-empty `error`** (timeouts, HTTP errors such as Wikidata **429**, etc.) are **removed** from the `providers` array so clients mostly see useful hits or clean “no match” rows. After the first round, if any provider returned a **title** (and artist when present), **Deezer** and **Discogs** get an extra **free-text** search when their code-based lookup missed.
